...
public class CollectionController {
    ...
    //   获取我的收藏列表
    @GetMapping("")
    public ResponseEntity getAllCollectionMusic(@RequestParam(name = "userId") Long userId) {
    ...
    public ResponseEntity getCollectionMusic(@PathVariable("id") Long id) {
        ...
        if(collection == null){
            return new ResponseEntity<>("没有找到该收藏单", HttpStatus.NOT_FOUND);
        }
        //每次点击歌单里的歌曲，播放量+1
//        collection.setPlayCount(collection.getPlayCount() + 1);
    ...
    public ResponseEntity recommendMusic(@PathVariable("userId") Long userId) {
        ...
        //找到自己评分高的50音乐
        List<Long> myLoveMusicIdList = collections.subList(0,50).stream().map(Collection::getMusicId).collect(Collectors.toList());
        ...
        //根据50首歌曲找到相应歌手Id，然后构造对象{歌手id,出现次数}
        ...
        List myLoveSignerIdListCount = new ArrayList();
        for (Long id:myLoveSignerIdList){
            Map<Long, Integer> myLoveSignerIdBoardMap = new HashMap<>();
            myLoveSignerIdBoardMap.put(id,Collections.frequency(myLoveSignerIdList,id));  //统计出现次数
            myLoveSignerIdListCount.add(myLoveSignerIdBoardMap);
        }

        //对hashMap排序，得到喜欢的歌手排行榜
        List<Map.Entry<Long, Integer>> list = new ArrayList<>(myLoveSignerIdBoardMap.entrySet());
        ...
        //找到最喜欢的歌手
        Long mostLovedSignerId = myLoveSignerIdBoardMap.entrySet();
        ...
        //6 热评
        List<Comment> comments = commentRepository.findByMusicIdOrderByLoveCount(music.getId());
        Long commentId = comments.get(0).getUserId();  //热评的人
